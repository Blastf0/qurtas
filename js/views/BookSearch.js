...
class BookSearchView {
  ...
  confirmAddBook(bookData) {
    if (!bookData.totalPages || bookData.totalPages <= 0) {
      const input = prompt(`"${bookData.title}" has an unknown page count. Please enter the total number of pages:`);
      if (input === null) return; // user cancelled
      const pages = parseInt(input);
      if (isNaN(pages) || pages <= 0) {
        showToast('Invalid page count. Book not added.', 'warning');
        return;
      }
      bookData = { ...bookData, totalPages: pages };
    }
    ...
    if (success) {
      ...
      showToast(`"${bookData.title}" is already in your library!`, 'warning');
    }
  }
